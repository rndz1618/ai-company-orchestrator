...
export default function Workflows() {
  ...
  const start = async (w: WorkflowTemplate) => {
    ...
    setError(null)
    try {
      const tasks = await api.workflows.start(w.id, w.company_id, prefix.trim() || undefined)
      alert(`Started workflow → ${tasks.length} task(s) created. Go to Tasks to run / advance.`)
    } catch (err: unknown) {
      setError(err instanceof Error ? err.message : String(err))
    } finally {
      setStartingId(null)
        ...
        <form onSubmit={save} className="space-y-3">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <button type="button" onClick={addStage} className="text-xs text-accent min-h-[36px] px-2">
                + Stage
              </button>
            </div>
            {stages.map((s, i) => (
              <div key={i} className="rounded-lg border border-border bg-surface p-3 space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  {stages.length > 1 && (
                    <button type="button" onClick={() => removeStage(i)} className="text-xs text-danger min-h-[32px]">
                      Remove
                    </button>
                  )}
                </div>
                <input
                  ...
                  className="w-full min-h-[40px] rounded-lg border border-border bg-surface-2 px-2 text-sm"
                />
                <input
                  required
                  ...
                  className="w-full min-h-[40px] rounded-lg border border-border bg-surface-2 px-2 text-sm"
                />
                <label className="flex items-center gap-2 text-xs min-h-[40px]">
                  <input
                    type="checkbox"
